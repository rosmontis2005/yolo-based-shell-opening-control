...
from project_paths import CAMERA_SCANS_DIR, PROJECT_ROOT

FRAME_DIR = CAMERA_SCANS_DIR / "frames"
MAX_CAMERA_INDEX = 10
...
def save_summary(results: list[dict]) -> tuple[Path, Path]:
    ...
    CAMERA_SCANS_DIR.mkdir(parents=True, exist_ok=True)
    json_path = CAMERA_SCANS_DIR / f"camera_scan_{timestamp}.json"
    txt_path = CAMERA_SCANS_DIR / f"camera_scan_{timestamp}.txt"
