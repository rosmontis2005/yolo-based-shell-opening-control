...
## 1. 结论先说
Arduino 不能直接运行或调用 `src/detect_once.py`。
...
本仓库已经准备好：
- `src/detect_once.py`：支持 `--json` 输出检测结果
- `src/control_servo.py`：运行检测并把结果发给 Arduino
...
## 5. 先测试检测脚本（可选）
```powershell
python src/detect_once.py --json
```
...
### 6.1 查看串口
```powershell
python src/control_servo.py --list-ports
```

### 6.2 运行一次检测并控制舵机
```powershell
python src/control_servo.py --port COM3 --baud 9600 --camera-index 1
```
...
### 6.3 只做检测不发串口
```powershell
python src/control_servo.py --dry-run --camera-index 1
```

## 7. 你问的“C++ 程序能调用 `src/detect_once.py` 吗？”
